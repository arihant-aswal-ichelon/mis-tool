    ...
    public function get_instagram_insight(Request $request){
        ...
        switch ($section) {
            ...
            case 'reporting':
                ...
                for ($i = 0; $i < 6; $i++) {
                    // Calculate the target month (current month minus $i months)
                    $targetTimestamp = strtotime("-$i months", $todayTimestamp);
                    
                    // Get the first day of the target month
                    $sinceDate = date('Y-m-01', $targetTimestamp);
                    
                    // For current month, use yesterday's date as untilDate
                    // For previous months, use last day of the month
                    if ($i == 0) {
                        $untilDate = date('Y-m-d', $yesterdayTimestamp);
                    } else {
                ...
                // Process the insights data for each month
                $reportingData = array();
                foreach($Dateresult as $date){
                    $IgsinceDate = $date['sinceDate'];
                    $IguntilDate = $date['untilDate'];
                    
                    $InstagramBusinessAccountArr = InstagramHelper::getInstagramReporting($this->fbVersion, $FbPageId, $FbPageToken, $instagramAccountId, $limit, $IgsinceDate, $IguntilDate);
                    if(!empty($InstagramBusinessAccountArr)){
                        $reportingData[] = array(
                            'sinceDate' => $IgsinceDate,
                            'untilDate' => $IguntilDate,
                            'data' => $InstagramBusinessAccountArr
                        );
                    }
                }

                $data['igData'] = $reportingData;
                
                return view("analysis.instagram.ig-reporting", compact("data", "section", "instagramAccountId", "client_id"));
                break;
            case 'programming':
                $data['from_section'] = "programming";

                $todayTimestamp = time();
                $Dateresult = array();

                // Loop through the last 3 months (excluding current month)
                for ($i = 1; $i <= 3; $i++) {
                    // Calculate the target month (current month minus $i months)
                    $targetTimestamp = strtotime("-$i months", $todayTimestamp);
                ...
                // Process the insights data for each month
                $reportingData = $processreportingData = array();
                foreach($Dateresult as $date){
                    $IgsinceDate = $date['sinceDate'];
                    $IguntilDate = $date['untilDate'];
                    
                    $InstagramBusinessAccountArr = InstagramHelper::getInstagramProgramming($this->fbVersion, $FbPageId, $FbPageToken, $instagramAccountId, $limit, $IgsinceDate, $IguntilDate);
                ...
                if(!empty($reportingData) && is_array($reportingData)){
                    $reportingDataEncode = json_encode($reportingData);
                    $processreportingData = InstagramHelper::processreportingData($reportingDataEncode);
                }

                $data['igData'] = $processreportingData;
                
                return view("analysis.instagram.ig-programming", compact("data", "section", "instagramAccountId", "client_id"));
                break;
            default:
